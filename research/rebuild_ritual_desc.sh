#!/bin/bash
# Rebuild the RITUAL_DESC table of descRitual.lua from per-entry block files (§T446).
#
...
# the file names. --split writes what descRitual.lua already has into <blockdir> as
...
MOD="$ROOT/Plugins/Sheets/World of Darkness 20th Anniversary Edition/descRitual.lua"
...
START=$(grep -n $'^\t\t\t\t\\["' "$MOD" | head -1 | cut -d: -f1)
END=$(grep -n -- '-- <<< RITUAL_DESC_END' "$MOD" | cut -d: -f1)
[ -n "$START" ] && [ -n "$END" ] || { echo "FATAL: markers not found in descRitual.lua" >&2; exit 1; }

if [ "$2" = "--split" ]; then
	head -n $((END-1)) "$MOD" | tail -n +"$START" > "$DIR/existing.lua"
...
head -n $((START-1)) "$MOD"  > "$DIR/.new"
cat "$DIR/.region"          >> "$DIR/.new"
tail -n +"$END" "$MOD"      >> "$DIR/.new"
cp "$DIR/.new" "$MOD"
...
[ "$(grep -c '^return {' "$MOD")" = "1" ] || { echo "FATAL: module does not open with exactly one 'return {'" >&2; exit 1; }
[ "$(tail -n 1 "$MOD")" = "};" ] || { echo "FATAL: module does not close with '};'" >&2; exit 1; }
grep -o $'^\t\t\t\t\\["[^"]*"' "$MOD" | sed 's/.*\["//;s/"$//' > "$DIR/.keys"
